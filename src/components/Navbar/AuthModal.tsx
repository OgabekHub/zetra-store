...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onLoginSuccess }) => {
  ...
  const [pendingUser, setPendingUser] = useState<{ name: string; email: string; isRegister: boolean } | null>(null);
  ...
  const logSecurityEvent = (event: string, email: string, status: 'success' | 'failed' | 'warning') => {
    ...
    localStorage.setItem('zetra-security-logs', JSON.stringify([newLog, ...logs]));
  };
  ...
  const createSession = (email: string) => {
    ...
    // Mark other sessions for this email as isCurrent: false
    const updatedSessions = sessions
      .filter((s: any) => s.email !== email || s.isCurrent === false)
    ...
    localStorage.setItem('zetra-sessions', JSON.stringify([newSession, ...updatedSessions]));
  };
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    if (loginPassword.length < 6) {
      toast.error(language === 'uz' ? "Parol kamida 6 ta belgidan iborat bo'lishi kerak!" : language === 'ru' ? "Пароль должен состоять минимум из 6 символов!" : "Password must be at least 6 characters long!");
      return;
    }
    
    // Simulate setting up 2FA Step
    ...
    logSecurityEvent("Kirish so'rovi: 2FA kod jo'natildi", loginEmail, 'warning');
    toast.success(language === 'uz' ? "Tasdiqlash kodi yuborildi! (Kod: 1998)" : language === 'ru' ? "Код подтверждения отправлен! (Код: 1998)" : "Verification code sent! (Code: 1998)");
  };
  ...
  const handleRegisterSubmit = (e: React.FormEvent) => {
    ...
    if (registerPassword !== registerConfirmPassword) {
      toast.error(language === 'uz' ? "Parollar mos kelmadi!" : language === 'ru' ? "Пароли не совпадают!" : "Passwords do not match!");
      return;
    }
    ...
    logSecurityEvent("Ro'yxatdan o'tish so'rovi: 2FA kod jo'natildi", registerEmail, 'warning');
    toast.success(language === 'uz' ? "Tasdiqlash kodi yuborildi! (Kod: 1998)" : language === 'ru' ? "Код подтверждения отправлен! (Код: 1998)" : "Verification code sent! (Code: 1998)");
  };
  ...
  const handleOtpSubmit = (e: React.FormEvent) => {
    ...
    if (otpCode === '1998') {
      if (pendingUser) {
        logSecurityEvent(pendingUser.isRegister ? "Ro'yxatdan muvaffaqiyatli o'tildi" : "Tizimga muvaffaqiyatli kirildi (2FA)", pendingUser.email, 'success');
        createSession(pendingUser.email);
        
        onLoginSuccess(pendingUser.name, pendingUser.email);
        toast.success(
          language === 'uz' 
            ? `Xush kelibsiz, ${pendingUser.name}!` 
            : language === 'ru' 
              ? `Добро пожаловать, ${pendingUser.name}!` 
      ...
      if (pendingUser) {
        logSecurityEvent("2FA kod xatosi kiritildi", pendingUser.email, 'failed');
      }
      toast.error(language === 'uz' ? "Tasdiqlash kodi xato kiritildi!" : language === 'ru' ? "Неверный код подтверждения!" : "Invalid verification code!");
    }
  };
  ...
  const handleResendOtp = () => {
    setCountdown(60);
    if (pendingUser) {
      logSecurityEvent("2FA kodi qayta so'raldi", pendingUser.email, 'warning');
    }
    toast.success(language === 'uz' ? "SMS kod qayta yuborildi! (Kod: 1998)" : language === 'ru' ? "СМС код отправлен повторно! (Код: 1998)" : "OTP code resent! (Code: 1998)");
  };
  ...
  const handleSocialLogin = (platform: string) => {
    ...
    setTimeout(() => {
      ...
      logSecurityEvent(`Ijtimoiy kirish (${platform})`, email, 'success');
      createSession(email);
    ...
    >
      ...
      >
        ...
        <div className="p-6 overflow-y-auto">
          {step === 'social_loading' && (
            <div className="p-8 text-center space-y-6 py-16 animate-fade-in flex flex-col items-center">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-slate-400 light:text-slate-500 max-w-xs">{socialPlatform} - {t('auth_social_popup_desc')}</p>
              </div>
            </div>
          )}
          ...
          {step === 'otp' && (
            <div className="p-4 space-y-6 animate-fade-in">
              <div className="text-center space-y-2">
                ...
                <p className="text-xs text-slate-400 light:text-slate-500 max-w-sm mx-auto leading-relaxed">{t('auth_2fa_subtitle')}</p>
                <span className="inline-block text-[10px] bg-slate-950/40 text-slate-400 light:bg-slate-100/50 light:text-slate-500 px-2.5 py-0.5 rounded-md font-mono mt-1 select-none animate-pulse">
                  Demo Code: 1998
                </span>
              </div>
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    onClick={() => {
                      setStep('form');
                      if (pendingUser) {
                        logSecurityEvent("2FA tasdiqlash bekor qilindi", pendingUser.email, 'warning');
                      }
                    }}
            ...
            <>
              {activeTab === 'login' ? (
                <form onSubmit={handleLoginSubmit} className="space-y-4">
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      ...
                      >
                        {language === 'uz' ? 'Parolni unutdingizmi?' : language === 'ru' ? 'Забыли пароль?' : 'Forgot password?'}
                      </button>
                    </div>
                    <div className="relative">
                      <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-500 light:text-slate-400">
                  ...
                  <div className="flex items-center">
                    ...
                    <label htmlFor="remember-me" className="ml-2 text-sm text-slate-400 light:text-slate-600 select-none cursor-pointer">
                      {language === 'uz' ? 'Meni eslab qol' : language === 'ru' ? 'Запомнить меня' : 'Remember me'}
                    </label>
                  </div>
                ...
                <form onSubmit={handleRegisterSubmit} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-400 light:text-slate-500 uppercase tracking-wider mb-2">
                      {language === 'uz' ? 'Parol yaratish' : language === 'ru' ? 'Создать пароль' : 'Create password'}
                    </label>
                    <div className="relative">
                      <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-500 light:text-slate-400">
                        <Lock className="w-5 h-5" />
                      ...
                      <input
                        ...
                        className="block w-full pl-11 pr-3 py-3 border border-slate-800 light:border-slate-200 rounded-2xl bg-slate-950/40 light:bg-white text-slate-200 light:text-slate-900 placeholder-slate-500 light:placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 text-sm transition-all"
                        placeholder={language === 'uz' ? 'Kamida 6 ta belgi' : language === 'ru' ? 'Минимум 6 символов' : 'At least 6 characters'}
                        required
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="block w-full pl-11 pr-3 py-3 border border-slate-800 light:border-slate-200 rounded-2xl bg-slate-950/40 light:bg-white text-slate-200 light:text-slate-900 placeholder-slate-500 light:placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 text-sm transition-all"
                        placeholder={language === 'uz' ? 'Parolni qayta kiriting' : language === 'ru' ? 'Повторите ввод пароля' : 'Confirm password'}
                        required
                      />
                    </div>
                  </div>
              ...
              <div className="mt-8">
                <div className="relative flex py-2 items-center">
                  ...
                  <span className="flex-shrink mx-4 text-xs font-semibold text-slate-500 light:text-slate-450 uppercase tracking-wider bg-transparent">
                    {language === 'uz' ? 'Yoki quyidagilar orqali' : language === 'ru' ? 'Или войти через' : 'Or sign in with'}
                  </span>
                  <div className="flex-grow border-t border-slate-800 light:border-slate-200"></div>
                </div>
